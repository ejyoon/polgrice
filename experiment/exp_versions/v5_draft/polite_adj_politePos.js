...
var sents = {
    utterances: {
        terrible: {
            sent_utterance: " SP said to LS: \"Your BB was terrible!\""
        },        
        bad: {
            sent_utterance: " SP said to LS: \"Your BB was bad!\""
        },        
        okay: {
            sent_utterance: " SP said to LS: \"Your BB was okay!\"" 
        },
        good: {
            sent_utterance: " SP said to LS: \"Your BB was good!\"" 
        },
        amazing: {
            sent_utterance: " SP said to LS: \"Your BB was amazing!\"" 
        },
//        not_bad: {
...
//    },
    ...
    LS_goals: {
        truth: {
            LS_goal: " And LS added, \"I want your honest opinion.\""
        },
        polite: {
            LS_goal: " LS looked very nervous, as LS was sensitive to criticism."
        }
    }
}
...
//    polite = sents["polites"][polite]["sent_polite"];
    ...
    LS_goal = sents["LS_goals"][LS_goal]["LS_goal"];
...
//    polite = polite.replace("BB",BB).replace("SP",SP).replace("SP",SP).replace("SP",SP).replace("SP",SP).replace("SP",SP).replace("SP",SP);
    ...
    LS_goal = LS_goal.replace("BB",BB).replace("SP",SP).replace("SP",SP).replace("SP",SP).replace("SP",SP).replace("SP",SP).replace("SP",SP).replace("LS",LS).replace("LS",LS).replace("LS",LS);
    ...
    return [utterance, context, state, precontext, question, LS_goal];
}
...
//      experiment.data.polite.push(trial.polite);
      ...
      experiment.data.LS_goal.push(trial.LS_goal);
...
//        polite = trial.polite;
        state = trial.state;
        LS_goal = trial.LS_goal;
        domain = trial.domain;
        context = trial.context;
        sent_materials = doSentSubs(sents, state, domain, utterance);
      showSlide("stage");
      $("#context").html(sent_materials[3] + sent_materials[2] + sent_materials[1]  + sent_materials[5] + sent_materials[0]);  
