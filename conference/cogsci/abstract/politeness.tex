...
Conveying information in a false or indirect manner in consideration of addressees' wants (i.e. being polite)
...
\emph{epistemic utility}, or utility of providing the addressee new and accurate information,
and \emph{social utility}, or utility of maintaining or boosting the addressee's self-image (being polite).
...
The latter is potentially misleading but gives the addressee what she might want to hear----in other words, it would be polite.
...
who has both an epistemic goal to improve the addressee's knowledge state as well as a social goal to minimize any potential damage to the hearer's (and the speaker's own) self-image, which they called \emph{face}.
In their analysis, if the speaker's intended meaning contains no threat to the speaker or addressee's face,
...
We test this model by gathering data about utterance interpretations and goal attributions in settings where the true state of the world carries affective consequences for the addressee.
...
\citeA{Goodman2013} define speaker utility by the amount of information a \emph{literal listener} would still not know about world state $s$ after hearing a speaker's utterance $w$, what we will call \emph{epistemic utility}:
...
We propose there is a second component to the speaker's utility related to the intrinsic value of the state in the eyes of the addressee\footnote{At this point, we do not differentiate state value to the addressee from state value to the speaker, though in many situations these could in principle be different.}, what we will call \emph{social utility}.
%
...
$$
%
where $V$ is a value function that maps states to subjective utility values---this captures the affective consequences for the addressee of being in state $s$.
%
...
%
%
The speaker chooses utterances $w$ given the state $s$ and his goal weights $\hat{\beta}$: 
\begin{equation}
...
Inferences of the actual rating for addressee's performance, or the true state, varied depending on speaker's goal and utterance (Figure \ref{fig:exp3}).
...
Communication is often examined from the perspective of successful information transfer from speaker to addressee.
In the social realm, however, communication also can serve the social function of making the addressee feel good and saving her face.
...
To test this proposal, we examined inferential judgments on a speaker's utterance, which was a potentially face-threatening evaluation of the addressee's performance.
