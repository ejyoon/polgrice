...
People even attribute ambiguous speech to a polite desire to hide a truth that could hurt the listener's self-image (e.g., \citeNP{bonnefon2009}).
...
To address this issue, \citeA{Brown1987} sought to redefine the notion of ``cooperative speaker'' as a speaker
...
\caption{\label{fig:exp3} Results from Experiment 2 (top) and model predictions (bottom). Average states inferred based on speaker's goal and utterance. Error bars represent 95\% confidence intervals. \mcf{FIXME}}
\end{centering}
\end{figure*}
...
Finally, more positive utterances led to inferences of more positive states given the goal to be nice, which indicates that people think a ``nice'' speaker is still being somewhat informative with respect to communicating what the true state is.
% Thus, a nice speaker probably used the word ``amazing'' to describe an okay presentation rather than a terrible one.
...
The MAP and 95\% HDI for $\lambda$ is 1.8 [1.08, 3.1]; for $\alpha$ is 9.1 [3.4, 9.9]. %; for $\phi$ is \red{0.09 [0.06, 0.13]}.
...
For ``trying to be nice'', the model puts a high weight on niceness but also some appreciable on honesty.
The model fits worse for the ``trying to be mean'' case; all that the model infers is that the speaker was not honest.
...
% \ndg{this discussion section isn't that useful -- maybe just move the relevant points into the forgoing results?}\mcf{agreed}

In sum, the model captured key aspects of our empirical findings, albeit with some discrepancies.First, unlike the model predictions that the inferred states given honesty goal would be more positive throughout,
...
Thus, participants might have reasoned that the speaker encountered an impossible situation and couldn't be cooperative by being nice, so instead decided to cooperate by being honest.

Second, people thought that a mean speaker saying ``[your cake] was amazing'' meant the true state was bad, which was in contrast to our model predictions.
...
\begin{figure}[t] % MCF; moved this down for pagination reasons.
\begin{centering}
\includegraphics[width=\columnwidth]{figures/goal-posterior.pdf}
...
\mcf{make the smoothing kernel wider---so not so bumpy---and maybe put the legend on the plot? Could take up less space.}}
\end{centering}
\end{figure}
...
\begin{table}[t]
\caption{\label{tab:lmer1}  Coefficient estimates from mixed-effects models predicting goal attributions in Experiment 3. \mcf{could make goal a column?}}
\begin{center}
\begin{tabular}{l r r r l}
\hline
Predictor  &  Value (SE) & \emph{t}-value\\
...
% For the honesty goal, the peak in the likelihood of honesty goal attribution occurred when the literal meaning of the utterance was close to the true state, based on the literal semantics, and was overall correlated with the literal semantics ($r = 0.86$).
% Attributions of niceness generally showed a slightly decreasing slope with improved true state given particular utterance (represented by each facet in Figure \ref{fig:exp2}).
