...
We assume a uniform prior distribution over possible states of the world $s\sim \text{UniformDraw} \{1, 2, 3, 4, 5\}$. 
...
We put uninformative priors on these and infer their credible values using Bayesian data analytic techniques \cite{LW2014}.
\begin{eqnarray}
\lambda \sim \text{Uniform}(0,20)\\
\alpha \sim \text{Uniform}(0, 5)
\end{eqnarray}
We ran X chains for Y iterations, discarding the first Z for burnin.
The 95\% Highest Probability Density Interval (HDI) for $\lambda$ is \red{[ ,]} and for $\alpha$ is \red{[, ]}.
