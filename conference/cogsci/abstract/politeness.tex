...
\title{Talking with tact: Polite language as a balance between informativity and kindness}
...
\caption{\label{fig:exp3} Results from Expt.~2 (left) and model predictions (center) for average states inferred based on a speaker's goal and utterance. Right: Full distribution of human responses vs. model predictions. Error bars represent 95\% confidence intervals.}
\end{centering}
\end{figure*}
...
The model also does not capture the interaction that a truthful positive utterance is both honest and nice, while a truthful negative utterance is honest but not that mean.
...
%Rather, when cost is tied to the social world, it's not clear when something is costly vs. not (e.g. is a speaker who says your project was ``bad'' when it was indeed bad is producing a more costly utterance because his utterance will make the listener feel bad?)
...
%With politeness, it is not clear that speakers want to communicate along a dimension of affect, but rather they would rather \emph{not communicate} along any epistemic dimension. 
%This possibility is given to the speaker who can use social utility as a motivating force for a speech act. 
