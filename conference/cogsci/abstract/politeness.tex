...
seemingly contradicts an optimal speaker's cooperative nature. 
...
the idea of politeness as a balance between informativity and kindness.
...
People interpret same words differently depending on the contexts, in a direction that allows charitable attribution of speaker's intention to save the listener's face (e.g., \citeNP{bonnefon2009}). 
...
We extend \citeA{Brown1987}'s theory to look at politeness as a balance between two goals, epistemic and social. 
...
%\ndg{take a little more time to say that formal models based on informativity have worked well, and we extend them here to incorporate the competing goal of kindness...}
...
In the current paper, we posited that speakers reason about two different goals, to be informative vs. to be kind, 
to determine how truthful or positively biased their utterances should be. 
To test this, we examined people's inferential judgments on utterances addressing evaluation of someone's performance that can boost or hurt that person's self-image. 
Specifically, we found that people attribute goals to speakers differentially based on 
...
Also, we found that people expect the actual deserved rating to be different 
depending on what the speaker said and whether the speaker's intended goal was to be honest, nice or mean. 
