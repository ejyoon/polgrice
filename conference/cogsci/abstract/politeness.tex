...
Conveying information in a false or indirect manner in consideration of a listener's wants (i.e., being polite)
...
Must we conclude people are not cooperative communicators? \citeA{Brown1987} recast the notion of a \emph{cooperative speaker} as one
...
a speaker will choose to be polite and produce more indirect utterances, keeping the intended meaning \emph{off the record}
...
Participants read scenarios and indicate their answer to each question by answering `No' or `Yes' (see Fig. \ref{fig:ex} for a screenshot of an example trial).The experiment can be viewed at: \url{http://langcog.stanford.edu/expts/EJY/polgrice/L2_J/polgrice_L2_J.html}.
...
The model presented here relates to other work done in game-theoretic pragmatics \cite{VanRooy2003} and in the Rational Speech-Acts theory \cite{Kao2014, Kao2015}.
\citeA{VanRooy2003} presents a game-theoretic analysis of polite requests (``Could you possibly take me home?'') to argue the purpose of polite language is to align the preferences of interlocutors. 
The analysis relies on the notion that polite language is \emph{costly} in a social way (e.g. reduced social status, incurred social debt to one's conversational partner) but it's not clear how the polite behavior explored in our experiments would be costly in anyway. Rather, when cost is tied to the social world, it's not clear when something is costly vs. not (e.g. is a speaker who says your project was ``bad'' when it was indeed bad is producing a more costly utterance because his utterance will make the listener feel bad?)
Our model derives its predictions by construing the speaker utility as a collection of possible goals (here, epistemic and social goals). The words themselves are not costly. 

Work in the Rational Speech-Acts theory has demonstrated nonliteral interpretations of figurative language by elaborating the set of possible \emph{Questions Under Discussion} \cite{Kao2014, Kao2015}.
Speakers may use language hyperbolically (``The latte costs a hundred bucks.''), for example, to communicate along the dimensions of affect (\emph{The speaker thinks the latte is expensive}).
Our approach is different in that we do not modify what question the speaker is addressing, but rather what utilities motivate him to act. 
