...
```{r}
litSemantics.results <- read.csv(paste(local.path, "model/data/literal_semantics_wNeg_20samples.csv",
                                       sep = ""))
```
...
var listener1 = cache(function(utterance, rsaParameters) {
  Infer({method: "enumerate"}, function(){
    ...
    // observe(S1, utterance, rsaParameters)
...
var dataAnalysis = function(){
  ...
  var goalWeightsAndPostPred = map(function(goal){
    ...
    var postPred = map(function(state){
      ...
      var utteranceData = _.pluck(_.where(data, {state: state, goal: goal}), "utterance"); // how to specify utterance with both utterance x utterance_prob from the data?
