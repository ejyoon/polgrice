...
var kernelFn = function(prevVal) {
  var concentration = 5;
  var x = prevVal < 0.25 ? 0.25 :
      prevVal > 0.75 ? 0.75 :
      prevVal
  var a = x * concentration;
  var b = (1 - x) * concentration;
  var contDist = Beta({a:a,b:b});
  var discProbs = map(function(bi){
    Math.exp(contDist.score(bi))
  }, weightBins)
  return Categorical({vs: weightBins, ps:discProbs})
};
...
write.csv(bda.utterance.results,paste(local.path,
                      "model/results/bda-politeRSA-speaker1GivenStateGoal_10000burn5000_test.csv",sep=""), # CHANGE FILE NAME AS NEEDED
          row.names=FALSE)
...
# ggsave("~/Desktop/politeDataModel.pdf", width = 10)
