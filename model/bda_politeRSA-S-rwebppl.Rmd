...
var speaker1 = cache(function(literalSemantics, exptCondInfo, rsaParameters) {
  Infer({method: "enumerate"}, function(){
    ...
    //var speakerUtility = speakerGoals.phi*eUtility+(1-speakerGoals.phi)*sUtility;
...
#                       "model/results/bda-politeRSA-speaker1GivenStateGoal-100000burn50000.csv",sep=""), # CHANGE FILE NAME AS NEEDED
#           row.names=FALSE)
...
#                         "model/results/bda-politeRSA-speaker1GivenStateGoal-postLongForm.csv",
#                         sep = ""))

bda.state.tidy2 <- bda.state.tidy %>%
  filter(!is.na(val)) %>%
  group_by(goal, state, positivity, utterance) %>%
  distinct(val, .keep_all = TRUE)

bda.state.tidy3 <- bda.state.tidy2 %>%     
  group_by(goal, state, positivity, utterance) %>%
  # summarise(val = mean(val)) %>%
  filter(!is.na(val)) %>%
  summarize(
...
ggplot(data=bda.state.tidy3, aes(x=positivity, y=MAP, fill=utterance)) +
  geom_bar(stat="identity", position=position_dodge()) +
...
```
```{r}
bda.state.tidy4 <- bda.state.tidy2 %>%     
...
ggplot(data=bda.state.tidy4, aes(x=state, y=MAP, col=positivity, group=positivity)) +
  # geom_bar(stat="identity", position=position_dodge()) +
...
cor(ms_all$MAP, ms_all$model_MAP)^2
```
