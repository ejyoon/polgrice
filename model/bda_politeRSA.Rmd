...
fig1<-ggplot(m.pp, aes(x=Utterance, y = MAP, color = Goal, group= Goal))+
  geom_point(size=2, position=position_dodge(0.3))+
  geom_line(position=position_dodge(0.3))+
  geom_errorbar(aes(ymin = credLow, ymax = credHigh),
                position=position_dodge(0.3), width=0.2)+
...
ggsave("~/Documents/research/polgrice/conference/cogsci/abstract/figures/state-inference.pdf", height =5, width = 5)


md.tidy<- left_join(m.pp, ds.tidy)
